...
namespace MusiBuy.Admin.Controllers
{
    public class HomeController : MusiBuy
    {
        ...
        public HomeController(IConfiguration config, ILogin login, IRolePrivileges rolePrivilegesRepository, IContentManagement content, IUser user, ICreatores creatores, IPostManagement postManagement)
        {
        ...
        public IActionResult Index(LoginViewModel loginviewModel)
        {
            ...
            UserViewModel objUserViewModel = _loginRepository.ValidateLogin(loginviewModel);

            if (objUserViewModel != null && objUserViewModel.Id > 0)
            {
                CurrentAdminUser User = new CurrentAdminUser();
                User.RoleName = objUserViewModel.RoleName ?? string.Empty;
                User.UserID = objUserViewModel.Id;
                User.RoleID = objUserViewModel.RoleId;
                ...
                User.IsAdminUser = objUserViewModel.IsAdminUser;
