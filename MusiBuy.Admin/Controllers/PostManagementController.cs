...
namespace MusiBuy.Admin.Controllers
{
    ...
    public class PostManagementController : Controller
    {
        ...
        private readonly IFrontUser _frontuser;
        private static int _totalCount = 0;

        public PostManagementController(IPostManagement postManagement, IConfiguration config, ICreatores creatoresRepository, IEnum enumrepo, IDropdown dropdown, IFrontUser frontuser)
        {
        ...
        {
            var result = _postManagementRepo.GetPostManagementList(CurrentAdminSession.User.UserID, searchValue);
        ...
        public ActionResult Create(PostManagementViewModel objValidatePost)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (CurrentAdminSession.IsAdminUser && CurrentAdminSession.UserID != 0)
                {
                    var CreatorDetail = _frontuser.GetFrontUserDetailById(objValidatePost.CreatorId);
                    objValidatePost.UserId = objValidatePost.CreatorId;
                    objValidatePost.UserTypeId = CreatorDetail.UserTypeId;
                    objValidatePost.RoleId = CreatorDetail.RoleId ?? 0;
                }
        ...
        public void BindDropdown(PostManagementViewModel objValidatePost)
        {
            ...
            var MediaTypedata = _enum.GetEnumsListByType((int)EnumTypes.PostMediaType);
            if (MediaTypedata.Count() > 0)
                objValidatePost.SelectMediaType = new SelectList(MediaTypedata.ToList(), "Id", "EnumValue");

            objValidatePost.SelectContentType = new SelectList(_dropdown.GetEnumListByType((int)EnumTypes.ContentType).ToList(), "Id", "EnumValue");
            objValidatePost.SelectCountry = new SelectList(_dropdown.GetCountry().ToList(), "Id", "Name");
            objValidatePost.SelectGenre = new SelectList(_dropdown.GetGenre().ToList(), "Id", "GenreName");
            //objValidatePost.SelectCategory = new SelectList(_dropdown.GetEnumListByType((int)EnumTypes.ContentType).ToList(), "Id", "EnumValue");
            objValidatePost.SelectType = new SelectList(_dropdown.GetEnumListByType((int)EnumTypes.ContentMusicProdcastType).ToList(), "Id", "EnumValue");
        }
        public IActionResult GetPodcastCategories()
        {
            return Json(_dropdown.GetEnumListByType((int)EnumTypes.ContentProdcastCategory));
        }

        public IActionResult GetSocialCategories()
        {
            return Json(_dropdown.GetEnumListByType((int)EnumTypes.ContentSocialMediaCategory));
        }
        public IActionResult GetMusicProducerTypes()
        {
            return Json(_dropdown.GetEnumListByType((int)EnumTypes.ContentMusicProdcastType));
        }
        ...
        public bool MediaFileValidation(PostManagementViewModel objValidatePost)
        {
            ...
            if (objValidatePost.MediaTypeId == (int)PostMediaTypeEnum.Audio && !contentType.StartsWith("audio/"))
            { ModelState.AddModelError("MediaFile", "Please upload a valid audio file."); return false; }
            else if (objValidatePost.MediaTypeId == (int)PostMediaTypeEnum.Video && !contentType.StartsWith("video/"))
            { ModelState.AddModelError("MediaFile", "Please upload a valid video file."); return false; }
            else if (objValidatePost.MediaTypeId == (int)PostMediaTypeEnum.Image && !contentType.StartsWith("image/"))
            { ModelState.AddModelError("MediaFile", "Please upload a valid image file."); return false; }
            else if (objValidatePost.MediaTypeId == (int)PostMediaTypeEnum.Text && !(contentType == "text/plain" || contentType.Contains("pdf") || contentType.Contains("word")))
