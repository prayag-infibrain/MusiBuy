...
namespace MusiBuy.Admin.Controllers
{
    ...
    public class RoleController : Controller
    {
        ...
        {
            var result = _roleRepository.GetRoleList(CurrentAdminSession.User.RoleID, roleValue);
        ...
        public ActionResult Create(RoleViewModel roleViewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                roleViewModel.CreatedBy = CurrentAdminSession.UserID;
        ...
        public ActionResult Edit(RoleViewModel roleViewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    roleViewModel.UpdatedBy = CurrentAdminSession.UserID;
                    roleViewModel.UpdatedOn = DateTime.UtcNow;
                    bool isUpdated = _roleRepository.SaveChanges(roleViewModel);
                    if (isUpdated)
                    {
                        if (roleViewModel.Id == CurrentAdminSession.RoleID)
                        {
