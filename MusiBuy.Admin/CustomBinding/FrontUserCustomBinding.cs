...
{
    public static class FrontUserCustomBinding
    {
        ...
        public static IQueryable<FrontUserViewModel> ApplyFiltering(this IQueryable<FrontUserViewModel> data, IList<IFilterDescriptor> filterDescriptors)
        {
            if (filterDescriptors.Any())
            {
                data = data.Where(ExpressionBuilder.Expression<FrontUserViewModel>(filterDescriptors));
            }
            return data;
        }

        public static IEnumerable ApplyGrouping(this IQueryable<FrontUserViewModel> data, IList<GroupDescriptor> groupDescriptors)
        {
            Func<IEnumerable<FrontUserViewModel>, IEnumerable<AggregateFunctionsGroup>> selector = null;
            ...
            foreach (var group in groupDescriptors.Reverse())
            {
                ...
                if (selector == null)
                {
                    ...
                    {
                        selector = orders => BuildInnerGroup(orders, o => o.Mobile, group.Member);
                    }
        ...
        private static Func<IEnumerable<FrontUserViewModel>, IEnumerable<AggregateFunctionsGroup>> BuildGroup<T>(Func<FrontUserViewModel, T> groupSelector, Func<IEnumerable<FrontUserViewModel>, IEnumerable<AggregateFunctionsGroup>> selectorBuilder, string Value)
        {
        ...
        private static IEnumerable<AggregateFunctionsGroup> BuildInnerGroup<T>(IEnumerable<FrontUserViewModel> group, Func<FrontUserViewModel, T> groupSelector, string Value)
        {
        ...
        public static IQueryable<FrontUserViewModel> ApplyPaging(this IQueryable<FrontUserViewModel> data, int currentPage, int pageSize)
        {
        ...
        public static IQueryable<FrontUserViewModel> ApplySorting(this IQueryable<FrontUserViewModel> data, IList<GroupDescriptor> groupDescriptors, IList<SortDescriptor> sortDescriptors)
        {
        ...
        private static IQueryable<FrontUserViewModel> AddSortExpression(IQueryable<FrontUserViewModel> data, Kendo.Mvc.ListSortDirection sortDirection, string memberName)
        {
            ...
            if (sortDirection == Kendo.Mvc.ListSortDirection.Ascending)
            {
                switch (UserEnum)
                {
                    ...
                    case UserFields.Phone:
                        data = data.OrderBy(order => order.Mobile);
                        break;
                }
            }
            else
