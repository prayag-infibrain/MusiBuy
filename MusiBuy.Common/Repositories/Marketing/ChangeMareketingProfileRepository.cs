...
{
    public class ChangeMareketingProfileRepository : IChangeMareketingProfile
    {
        #region Member Declaration
        private readonly MusiBuyDB_Connection _Context;

        public ChangeMareketingProfileRepository(MusiBuyDB_Connection context)
        {
            _Context = context;
        }
        #endregion
        ...
        {
            var userData = (from u in _Context.MarketingUsers
                            where u.Id == userID
        ...
        {
            return _Context.MarketingUsers.Where(u => u.Email == email && u.Id != userId).Any();
        }
        #endregion
        ...
        {
            MarketingUser? tbl = _Context.MarketingUsers.Where(e => e.Id == changeProfileViewModel.UserId).FirstOrDefault();
            if(tbl == null)
            {
                tbl = new MarketingUser();
            }            
