...
namespace MusiBuy.Marketing.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public UserController(IUser user, IRole role, IConfiguration config, ICommonSetting common)
        {
        ...
        {
            var result = _userRepository.GetUserList(CurrentUserSession.User.UserID, searchValue);
        ...
        {
            UserViewModel objValidateUser = new UserViewModel();
        ...
        [HttpPost]
        [ValidatePageAccess(GlobalCode.Actions.Create)]
        public ActionResult Create(UserViewModel objValidateUser)
        {
            if (ModelState.IsValid)
            {
                ...
                if (isSaved)
                    ...
                    return RedirectToAction("Index", "User", new { msg = "not added" });
            }
            else
            {
                objValidateUser.SelectRole = new SelectList(_roleRepository.GetRoleDropDownList(true), "value", "name");
        ...
        {
            UserViewModel objValidateUser = _userRepository.GetUserDetailsByID(id);
        ...
        [HttpPost]
        [ValidatePageAccess(GlobalCode.Actions.Edit)]
        public ActionResult Edit(UserViewModel objValidateUser)
        {
            if (ModelState.IsValid)
            {
                ...
                if (CurrentUserSession.UserID == objValidateUser.Id)
                {
                    CurrentAdminUser currentUser = CurrentUserSession.User;
                    currentUser.FirstName = (objValidateUser.FirstName ?? string.Empty).Trim();
                    currentUser.LastName = (objValidateUser.LastName ?? string.Empty).Trim();
                    currentUser.RoleID = objValidateUser.RoleId;
                    currentUser.UserName = (objValidateUser.Username ?? string.Empty).Trim();
                ...
                if (isUpdated)
                    ...
                    return RedirectToAction("Index", "User", new { msg = "not updated" });
        ...
        public ActionResult Delete(int[] chkDelete)
        {
            try
            {
                if (chkDelete.Length > 0)
                {
                    ...
                    return RedirectToAction("Index", "User", new { msg = "not deleted" });
                }
                else
                {
